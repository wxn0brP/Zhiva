...
*   `icon` (string): A path to an icon file (`.png`) to be used for the shortcut on Linux and macOS.
*   `win_icon` (string): A path to an icon file (`.ico`) to be used for the shortcut on Windows.

---

### Application Serving

You can define how your application is served. You can either run a full-stack application or a simple static server.

*   **Full-stack (Default)**: If no `static` property is defined, Zhiva expects a `start` script in your `package.json`. Zhiva will run your server to handle all file serving and API logic.
...
*   **Static Server (`static`)**: If the `static` property is present, Zhiva will treat your application as a static web project. Instead of running a start command, it will launch a built-in static server with the configuration you provide. This is ideal for simple websites or applications built with frameworks that output static files (like Vite, React, or static site generators).
    ...
    *   `redirects` (object): A mapping of URL paths to other paths for HTTP redirects. Example: `{"/home": "/"}` will redirect users from `/home` to the root.
